...
class TestExecutor {
  ...
  async run(filterIds, options) {
    ...
    const filePaths = testFiles.map((f) => path.join(this.testsDir, f)).join(' ');
    const envPrefix = options.reuseContext ? 'PW_TEST_REUSE_CONTEXT=1 ' : '';
    const cmd = `${envPrefix}npx playwright test ${filePaths} --config=${configPath}`;

    console.log('テストを実行中...\n');

    try {
      childProcess.execSync(cmd, { cwd: this.projectDir, stdio: 'inherit' });
